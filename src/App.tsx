...
useEffect(() => {
  ...
  const handleRsvpSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
     ...
     // Attempt to POST the same payload to the webhook (Pipedream) endpoint, or fall back to Apps Script
      const payload = {
        name: rsvpName,
        attendance: rsvpAttendance === 'yes' ? 'Joyfully accepts' : 'Regretfully declines',
        message: rsvpMessage.trim() || '(none)',
      ...
      if (webhookUrl) {
        try {
          ...
          if (!webhookResp.ok) {
            console.warn('Webhook POST returned', webhookResp.status);
          }
        } catch (err) {
  ...
  return (
    <main className="site-shell">
     <div className={`invitation-cover ${isOpen ? 'invitation-cover--open' : ''}`} aria-hidden={isOpen}>
      ...
      <header className="topbar"><button className="brand-mark" onClick={() => scrollTo('home')} aria-label="Back to top"><img src="/images/cc-logo-white.png" alt="" /></button><nav className={menuOpen ? 'nav-links nav-links--visible' : 'nav-links'}><button onClick={() => scrollTo('story')}>Our story</button><button onClick={() => scrollTo('details')}>The details</button><button onClick={() => scrollTo('entourage')}>Entourage</button><button onClick={() => scrollTo('gallery')}>Gallery</button><button onClick={() => scrollTo('faqs')}>FAQs</button></nav><button className="menu-button" onClick={() => setMenuOpen(!menuOpen)} aria-label="Toggle menu">{menuOpen ? <X /> : <Menu />}</button></header>
      

      <section className={`hero ${isOpen ? 'hero--introduced' : ''}`} id="home"><div className="hero-image" style={{ backgroundImage: `url(${heroBgImage})` }} /><div className="hero-overlay" /><div className="hero-content"><h2 className="hero-names" aria-label="Charlon & Chilzia"><span className="typed-name typed-name--first" aria-hidden="true">Charlon</span><span className="typed-join" aria-hidden="true">&</span><i className="typed-name typed-name--second" aria-hidden="true">Chilzia</i></h2><div className="hero-meta"><span>January 30, 2027</span><span className="meta-rule" /><span>Naga City, Camarines Sur</span></div><button className="text-link light" onClick={openRsvp}>RSVP</button></div><div className="hero-vertical">CHARLON & CHILZIA <span>•</span> 2027</div></section>

      <section className="intro-section section-pad reveal-on-scroll" id="story"><video className="flower-bloom flower-bloom--story" src="/images/blooming-flowers.mp4" autoPlay muted loop playsInline preload="metadata" aria-hidden="true" /><div className="section-label"><span>01</span><span className="label-line" /><span>ABOUT</span></div><div className="story-grid"><div><p className="eyebrow blue">Ecclesiastes 3:11</p><h2 className="display-heading">He has made<br /><i>everything beautiful in its time.</i></h2><p className="body-copy">A chance meeting. A thousand little moments. A lifetime to go.</p><p className="body-copy">Somehow, in the middle of ordinary days, we found something extraordinary. What started as a quiet hello became a life we never want to stop choosing.</p><div className="signature">
  <video
